...
$.fcdp = {
    ...
    buildUI: function(input) {
        ...
        if (hasDatePicker) {
            ...
            ds.append('<i class="fi-calendar"></i><input type="text" class="value">');
            sel.append(ds);
            sel.addClass('date');
        ...
        var opts = {
            calendar: cal,
            formats: {
                'date': input.is('[data-date-format]') ? input.data('date-format') : '%A: %N %-d, %Y',
    ...
    buildTime: function(opts) {
        if (tp = opts.timePicker) {
            ...
            var ctlHour = $('<div class="value-control hour"><label>Uur</label><a class="value-change up"><span></span></a><input type="text" class="display" value="12" /><a class="value-change down"><span></span></a></div>');
    ...
    wireupTime: function(opts) {
        if (tp = opts.timePicker) {
            ...
            this.wireupTimeValueControl(hour, 0, 23, 2);
...
/*            var second = parseInt(tp.find('.value-control.second').find('input.display').val());
    ...
    buildCalendar: function(opts) {
        ...
        if (dp = opts.datePicker){
            ...
            header.append('<div class="month">' + workingDate.format('%N %Y') + '</div>');

            dp.append(header);

            var labels = $('<div class="week labels"></div>');
            for (i=0;i<7;i++) {
                var ls = ['Zo','Ma','Di','Wo','Do','Vr','Za'];
